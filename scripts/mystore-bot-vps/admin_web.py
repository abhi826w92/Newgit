...
async def auth_middleware(request, handler):
    ...
    if (
        ...
        or path == "/api/proxy/image"
        or path.startswith("/api/proxy/image")
...
def _sanitize_app_media(app_dict):
    """Ensure icon and logo fields are clean, non-None string URLs, and mirrored."""
    ...
    if icon.lower() in ("none", "null", "undefined", ""):
        icon = ""
    if logo.lower() in ("none", "null", "undefined", ""):
        logo = ""

    valid_img = icon or logo
    if valid_img:
        item["icon"] = valid_img
        item["logo"] = valid_img
    else:
    ...
    if isinstance(raw_shots, str):
        parts = [p.strip() for p in re.split(r'[\n,]+', raw_shots) if p.strip()]
        cleaned_shots = [p for p in parts if p.startswith("http://") or p.startswith("https://") or p.startswith("/")]
    elif isinstance(raw_shots, list):
        for s in raw_shots:
            if isinstance(s, str) and s.strip() and (s.strip().startswith("http://") or s.strip().startswith("https://") or s.strip().startswith("/")):
                cleaned_shots.append(s.strip())
    item["screenshots"] = cleaned_shots
...
async def handle_get_apps(request):
    ...
    cleaned_apps = [_sanitize_app_media(a) for a in apps]
    return web.json_response({"apps": cleaned_apps})
...
async def handle_get_pinned(request):
    ...
    cleaned_pinned = [_sanitize_app_media(a) for a in pinned]
    return web.json_response({"pinned": cleaned_pinned})
...
async def handle_move_pinned(request):
    ...
    pinned = await asyncio.to_thread(firebase_mgr.get_pinned_apps)
    cleaned_pinned = [_sanitize_app_media(a) for a in pinned]
    return web.json_response({"success": ok, "pinned": cleaned_pinned})
...
def _get_cache_path_for_url(url):
    h = hashlib.sha256(url.encode()).hexdigest()
    return os.path.join(IMAGE_CACHE_DIR, f"{h}.jpg")

async def handle_proxy_image(request):
    """High performance disk-cached image proxy for CDN/ImgBB hotlink bypass and ISP unblocking."""
    raw_url = request.query.get("url", "").strip()
    if not raw_url:
        return web.Response(text="Missing url parameter", status=400)
    
    if not (raw_url.startswith("https://") or raw_url.startswith("http://")):
        return web.Response(text="Invalid URL scheme", status=400)

    cache_path = _get_cache_path_for_url(raw_url)

    # Check disk cache first (instant local SSD read in <2ms!)
    ...
    try:
        ...
        content_type, data = await asyncio.to_thread(_fetch_and_save)
        ...
        return web.Response(text=f"Image fetch error: {str(e)}", status=502)
...
async def handle_spa_fallback(request):
    ...
    if tail:
        candidate = os.path.abspath(os.path.join(WEB_DIST_DIR, tail))
        if os.path.commonpath([WEB_DIST_DIR, candidate]) == WEB_DIST_DIR and os.path.exists(candidate) and os.path.isfile(candidate):
...
def setup_routes(app):
    ...
    app.router.add_get("/api/proxy/image", handle_proxy_image)
